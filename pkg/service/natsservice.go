...
import (
	...
	"github.com/vedadiyan/goal/pkg/service/context"
...
func (t NATSService[TReq, TRes, TFuncType]) handler(msg *nats.Msg) {
	...
	context := context.NewNatsContext(t.conn, insight, msg, t.options.onerror, t.options.onsuccess)
	request := t.newReq()
	insight.OnFailure(func() {
		context.Error(map[string]string{"status": "FAIL:RECOVERED"})
	})
	...
	if t.options.isCached {
		...
		if err != nil {
			insight.Error(err)
			context.Error(map[string]string{"status": "FAIL:REQUEST:HASH"})
			return
		}
		requestHash = _requestHash
		value, err := (*t.bucket).Get(requestHash)
		if err == nil {
			context.Success(value.Value(), map[string]string{"status": "SUCCESS"})
			return
		}
	}
	if len(msg.Data) > 0 {
		...
		if err != nil {
			insight.Error(err)
			context.Error(map[string]string{"status": "FAIL:DECODE"})
			return
		}
	}
	...
	if err != nil {
		insight.Error(err)
		context.Error(map[string]string{"status": "FAIL:HANDLE", "error": err.Error()})
		return
	}
	bytes, err := t.codec.Encode(msg.Subject, response)
	if err != nil {
		insight.Error(err)
		context.Error(map[string]string{"status": "FAIL:ENCODE"})
		return
	}
	if t.options.isCached {
	...
	context.Success(bytes, map[string]string{"status": "SUCCESS"})
}
